...
> **注記: このドキュメントは現状の実装と乖離している可能性があります。**
> クリーンアーキテクチャ移行時(2025年7月頃)に書かれた手順であり、
> 本文中で参照している `test-clean-architecture.js` などの一時テストスクリプトはすでに削除されています。
> テスト基盤は別Issueで Vitest の導入が進行中のため、本ドキュメントは参考情報として残しています。
